...
class Lvgshop_custom_bg {
	...
	public function _add_controls( $element, $section_id, $args ) {
		if ( ('section' === $element->get_name() && 'section_background' === $section_id) || ('column' === $element->get_name() && 'section_style' === $section_id)) {
			...
			$element->start_controls_section(
				'lvgshop_code_bg_section',
				[
					'label' => __( 'Lvgshop :: Custom Background', 'lvgshop-core' ),
					'tab'   => Controls_Manager::TAB_STYLE
				]
			);
			
				$element->add_control(
			'lvgshop_darkmode_enabled',[
			...
			$element->add_control(
			'lvgshop_custombg_enabled', [
				'label'        => __( 'Enable Code Background', 'lvgshop-core' ),
			...
			$element->add_control(
			'lvgshop_thumbnail_enabled', [
				'label'        => __( 'Featured Image', 'lvgshop-core' ),
				'type'         => Controls_Manager::SWITCHER,
				'default'      => 'no',
				'label_on'     => __( 'Yes', 'lvgshop-core' ),
				'label_off'    => __( 'No', 'lvgshop-core' ),
	...
	function _before_render( \Elementor\Element_Base $element ) {
		...
		$custom_code_enable = $settings['lvgshop_custombg_enabled'];
		$lvgshop_thumbnail_enabled = $settings['lvgshop_thumbnail_enabled'];
		$lvgshop_darkmode_enabled = $settings['lvgshop_darkmode_enabled'];
		$lvgshop_darkmode_inner= $settings['lvgshop_darkmode_inner'];
		global $post;
		 $feat_image = wp_get_attachment_url( get_post_thumbnail_id($post->ID) );
		
		if ($lvgshop_darkmode_inner=="enable"){
		    $element->add_render_attribute( '_wrapper', [
		'class' => 'has_lvgshop_elementor_inner_bg',
	]);
		}
		
		
		
		if ( $lvgshop_darkmode_enabled == 'dark' ) {
		    		$element->add_render_attribute( '_wrapper', [
		'class' => 'has_lvgshop_dark_bg',
	]);
		} elseif($lvgshop_darkmode_enabled == 'dark-alt' ){
		    	$element->add_render_attribute( '_wrapper', [
		'class' => 'has_lvgshop_dark_alt_bg',
	]);
		} elseif($lvgshop_darkmode_enabled == 'dark-sec' ){
		    	$element->add_render_attribute( '_wrapper', [
		'class' => 'has_lvgshop_dark_sec_bg',
	]);
		}
		...
		$element->add_render_attribute( '_wrapper', [
		'class' => 'has_lvgshop_custom_bg',
		'style' => $custom_code,
	] );
		} elseif ($lvgshop_thumbnail_enabled =='yes'){
		    
		      global $post;
		    $feat_image = wp_get_attachment_url( get_post_thumbnail_id($post->ID) );
		$custom_code = 'background:url('.$feat_image.');';
		$element->add_render_attribute( '_wrapper', [
		'class' => 'has_lvgshop_custom_bg_thumbnail',
		'style' => $custom_code,
	] );
...
<#
if ( settings.lvgshop_custombg_enabled ) {
		var html = '<div class="lvgshop-custom-bg-wrapper" style="' + settings.custom_bg + '">';

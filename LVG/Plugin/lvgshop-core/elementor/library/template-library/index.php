...
if ( ! class_exists('lvgshop_Lib_Library')) {

	class lvgshop_Lib_Library {
		...
		private function __construct() {
			...
			self::$plugin_data = array(
			 'root_file' =>  __FILE__,
			 'pro-link' => '#',
			 'remote_site' => 'https://api.themepreview.xyz/lvgshop/',
			 'all_endpoint' => 'lvgshop_lib_all',
			 'single_endpoint' => 'lvgshop_single_lib'
			);
			...
			add_action( 'wp_ajax_lvgshop_lib_reload_template', array($this,'reload_library'));
		...
		public function editor_script(){
			...
			wp_enqueue_script( 'lvgshop-lib-library',  plugins_url( '/assets/js/elementor-manage-library.js', __FILE__));
			wp_enqueue_script( 'masonry');
			wp_enqueue_style( 'lvgshop_lib',  plugins_url( '/assets/css/style.css', __FILE__ ) );
		}

		function reload_library(){

			lvgshop_Lib_Activation::init();
		}
		...
		function ajax_data(){
			...
			$nav = '';
			$products = get_option('lvgshop_lib_library');
			...
			if ( is_array($products) ){
				...
				if (count($paged_products)) {
					foreach ($paged_products as $product) {
						...
						if( $product['pro'] && !class_exists('lvgshop_Lib_Pro') ){

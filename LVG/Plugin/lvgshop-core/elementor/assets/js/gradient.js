!(function ($) {
    window.addEventListener('elementor/init', () => {
        var lvgshopgradient = elementor.modules.controls.BaseData.extend({
            onReady() {
                var self = this;
                this.$el.find('.lvgshop-gradient-picker-el').each(function (i, el) {
                    let $el = $(el);
                    let $preview = $(el)
                        .closest('.lvgshop-gradient-picker-container')
                        .find('.lvgshop-gradient-picker-preview');
                    var swType = $(el)
                        .closest('.lvgshop-gradient-picker-container')
                    ...
                    var res = $(el)
                        ...
                        .find('.lvgshop_param_val:first');

                    const gp = new Grapick({
                        el: el,
                        colorEl: '<input class="lvgshop-colorpicker"/>',
                    });
                    gp.setValue(res.val());
                    if (gp.getDirection()) {
                        let dirVal = gp.getDirection();
                        dirVal = lvgshopCleanGradientDirection(dirVal);
                        swAngle.val(dirVal);
                    }
                    if (gp.getType()) {
                    ...
                    hs.forEach(function (handler) {
                        const el = handler.el;
                        const $el = $(el).find('.lvgshop-colorpicker');
                        $el.spectrum({
                    ...
                    swAngle.on('change keydown paste input', function (e) {
                        ...
                        newVal = lvgshopCleanGradientDirection(newVal);
                        gp && gp.setDirection(newVal || 'right');
                    });
                });

                function lvgshopCleanGradientDirection(dirVal) {
            ...
            saveValue() {
                this.setValue(this.$el.find('.lvgshop_param_val:first').val());
            },
            onBeforeDestroy() {
                this.saveValue();
            },
        ...
        elementor.addControlView('lvgshop_gradient_selector', lvgshopgradient);

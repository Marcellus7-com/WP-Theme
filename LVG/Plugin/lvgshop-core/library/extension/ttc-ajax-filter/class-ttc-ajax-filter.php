...
class Ttc_Ajax_Filter
{
    ...
    public function __construct() {
        ...
        $this->plugin_name = 'ttc-ajax-filter';
        $this->version = '0.5.2';
        ...
        add_shortcode('ttc_ajax_filter', [$this, 'create_shortcode']);
    }
    ...
    public function add_scripts() {
        ...
        $script_variables = [
            ...
            'timeoutMessage' => __('It took to long the get the posts. Please reload the page and try again.', 'ttc-ajax-filter'),
            'serverErrorMessage' => __('Got no response. Please reload the page and try again.', 'ttc-ajax-filter'),
        ];
        ...
        wp_enqueue_script('ajax-filter', plugins_url('/assets/js/ttc-ajax-filter.js', __FILE__), [], '', true);
        wp_enqueue_style('ajax-filter', plugins_url('/assets/css/ttc-ajax-filter.css', __FILE__), []);
    ...
    protected function validate_attributes($attributes) {
        ...
        $is_post_type_viewable = apply_filters('ttc_ajax_filter_is_post_type_viewable', $this->is_every_post_type_viewable($attributes['post_type']), $attributes);
        $is_post_status_viewable = apply_filters('ttc_ajax_filter_is_post_status_viewable', $this->is_every_post_status_viewable($attributes['post_status']), $attributes);

        if (!$is_post_type_viewable || !$is_post_status_viewable) {
            return new WP_Error('Posts not viewable', __("Something went wrong. The posts you've requested does not exist or is not viewable.", 'ttc-ajax-filter'));
        }

        // don't allow orderby values that are not supported
        if (!in_array($attributes['orderby'], $this->allowed_orderby_values)) {
            return new WP_Error('Invalid orderby attribute', __("Something went wrong. The posts could not be sorted with the given orderby method.", 'ttc-ajax-filter'));
        }
    ...
    protected function query_posts($args, $shortcode_attributes) {
        $query_args = apply_filters('ttc_ajax_filter_query_args', $args, $shortcode_attributes);
    ...
    protected function get_termlist($taxonomies) {
        $list = [];
        $filternumberebl = cs_get_option('filter_tax_number_ebl');
        $numbertx = cs_get_option('number_of_tax');
    ...
    public function process_filter_change() {
        ...
        if ($response) {
            ...
            wp_send_json_error(__('Oops, something went wrong. Please reload the page and try again.', 'ttc-ajax-filter'));
        }
        die();
    }
    ...
    public function get_filter_posts($args, $attributes) {
        ...
        $fpstyle = cs_get_option('lvgshop-archive-style');
    ...
    protected function get_post_type_plural_name($post_type) {
        ...
        return $post_type_object ? strtolower($post_type_object->labels->name) : __('posts', 'ttc-ajax-filter');
    }
     ...
     * 2. /plugins/ttc-ajax-filter/templates/$template_name.
     *
    ...
    public function get_local_template($template_name) {
        ...
        $template = locate_template('ttc-ajax-filter/' . $template_name);
        $template = apply_filters('ttc_ajax_filter_template_name', $template, $template_name);

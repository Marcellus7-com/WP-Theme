<?php
namespace Lvgshop\VariationSwatches\Admin;

defined( 'ABSPATH' ) || exit;

use Lvgshop\VariationSwatches\Helper;
use Lvgshop\VariationSwatches\Plugin;

class Product_Data {
	const META_NAME = 'lvgshop_variaiton_swatches';
	 ...
	 * @static
	 *
	 * @var Lvgshop\VariationSwatches\Admin\Product_Data
	 */
	 ...
	 * @static
	 *
	 * @return Lvgshop\VariationSwatches\Admin\Product_Data An instance of the class.
	 */
	...
	public function __construct() {
		...
		add_action( 'wp_ajax_lvgshop_variation_swatches_add_term', [ $this, 'ajax_add_new_attribute_term' ] );
	...
	public function product_option_terms( $taxonomy, $index, $attribute ) {
		...
		<select multiple="multiple" data-placeholder="<?php esc_attr_e( 'Select terms', 'lvgshop-variation-swatches' ); ?>" class="multiselect attribute_values wc-enhanced-select" name="attribute_values[<?php echo esc_attr( $index ); ?>][]">
			<?php
			$args      = [
		...
		<button class="button plus select_all_attributes"><?php esc_html_e( 'Select all', 'lvgshop-variation-swatches' ); ?></button>
		<button class="button minus select_no_attributes"><?php esc_html_e( 'Select none', 'lvgshop-variation-swatches' ); ?></button>
		<button class="button fr plus add_new_attribute_with_swatches" data-type="<?php echo esc_attr( $taxonomy->attribute_type ) ?>"><?php esc_html_e( 'Add new', 'lvgshop-variation-swatches' ); ?></button>
	...
	public function swatches_tab( $tabs ) {
		$tabs['lvgshop_variation_swatches'] = [
			'label'    => esc_html__( 'Swatches', 'lvgshop-variation-swatches' ),
			'target'   => 'lvgshop_variation_swatches_data',
	...
	public function swatches_panel() {
		...
		<div id="lvgshop_variation_swatches_data" class="panel woocommerce_options_panel wc-metaboxes-wrapper hidden">
			<div id="lvgshop_variation_swatches_data_inner" class="lvgshop-variation-swatches-product-data  wc-metaboxes">
				<?php
				...
				foreach ( $attributes as $attribute ) {
					...
					<div data-taxonomy="<?php echo esc_attr( $attribute->get_taxonomy() ); ?>" class="wc-metabox closed" rel="<?php echo esc_attr( $attribute->get_position() ); ?>">
						<h3>
							<div class="handlediv" title="<?php esc_attr_e( 'Click to toggle', 'lvgshop-variation-swatches' ); ?>"></div>
						...
						<div class="wc-metabox-content hidden">
							<div class="options_group">
								...
								woocommerce_wp_select( [
									'id'            => 'lvgshop_variation_swatches[' . $attribute_name . '][type]',
									'class'         => 'select',
									'wrapper_class' => 'lvgshop-variaton-swatches__type-field',
									'label'         => esc_html__( 'Type', 'lvgshop-variation-swatches' ),
									'description'   => sprintf( esc_html__( 'The default type is: %s', 'lvgshop-variation-swatches' ), $types[ $attribute_type ] ),
									'value'         => $attribute_swatches['type'],
									'options'       => array_merge( [ '' => esc_html__( 'Default', 'lvgshop-variation-swatches' ) ], $types ),
								] );
								?>
							</div>
							...
							<div class="options_group">
								<?php
								woocommerce_wp_select( [
									'id'            => 'lvgshop_variation_swatches[' . $attribute_name . '][shape]',
									'class'         => 'select',
									'wrapper_class' => 'lvgshop-variaton-swatches__shape-field',
									'label'         => esc_html__( 'Shape', 'lvgshop-variation-swatches' ),
									'description'   => sprintf( esc_html__( 'The default shape is: %s', 'lvgshop-variation-swatches' ), $shapes[ $default_settings['shape'] ] ),
									'value'         => $attribute_swatches['shape'],
									'options'       => array_merge( [ '' => esc_html__( 'Default', 'lvgshop-variation-swatches' ) ], $shapes ),
								] );
								?>
							</div>
							...
							<div class="options_group">
								<?php
								woocommerce_wp_select( [
									'id'            => 'lvgshop_variation_swatches[' . $attribute_name . '][size]',
									'class'         => 'select',
									'wrapper_class' => 'lvgshop-variaton-swatches__size-field',
									'label'         => esc_html__( 'Size', 'lvgshop-variation-swatches' ),
									'value'         => $attribute_swatches['size'],
									'options'       => [
										''       => esc_html__( 'Default', 'lvgshop-variation-swatches' ),
										'custom' => esc_html__( 'Custom', 'lvgshop-variation-swatches' ),
									],
								] );
								?>
								<p class="form-field form-field--custom-size dimensions_field <?php echo 'custom' != $attribute_swatches['size'] ? 'hidden' : '' ?>">
									<span class="wrap">
										<input type="text" name="lvgshop_variation_swatches[<?php echo esc_attr( $attribute_name ) ?>][custom_size][width]" value="<?php echo esc_attr( $attribute_swatches['custom_size']['width'] ) ?>" size="5" placeholder="<?php esc_attr_e( 'Width', 'lvgshop-variation-swatches' ) ?>">
										<input type="text" name="lvgshop_variation_swatches[<?php echo esc_attr( $attribute_name ) ?>][custom_size][height]" value="<?php echo esc_attr( $attribute_swatches['custom_size']['height'] ) ?>" size="5" placeholder="<?php esc_attr_e( 'Height', 'lvgshop-variation-swatches' ) ?>">
									</span>
								</p>
							</div>
	...
	protected function swatches_metabox( $args ) {
		...
		foreach ( $options as $option ) {
			...
			switch ( $args['type'] ) {
				case 'color':
					printf(
						'<span class="lvgshop-variation-swatches__field-color">
							<span class="label">%s</span><br>
							<input type="text" name="%s" value="%s">
						</span>',
						esc_html( $label ),
						esc_attr( "lvgshop_variation_swatches[$attribute_name][swatches][$name][color]" ),
				...
				case 'image':
					...
					printf(
						'<span class="lvgshop-variation-swatches__field-image">
						...
						esc_html( $label ),
						esc_html__( 'Edit', 'lvgshop-variation-swatches' ),
						empty( $values['image'] ) ? 'hidden' : '',
						esc_html__( 'Delete Image', 'lvgshop-variation-swatches' ),
						esc_html__( 'Remove', 'lvgshop-variation-swatches' ),
						esc_attr( "lvgshop_variation_swatches[$attribute_name][swatches][$name][image]" ),
				...
				case 'label':
					printf(
						'<span class="lvgshop-variation-swatches__field-label">
							<input type="text" name="%s" value="%s"><br>
							<span class="label">%s</span>
						</span>',
						esc_attr( "lvgshop_variation_swatches[$attribute_name][swatches][$name][label]" ),
	...
	public function process_product_swatches_meta( $post_id ) {
		if ( ! isset( $_POST['lvgshop_variation_swatches'] ) || ! is_array( $_POST['lvgshop_variation_swatches'] ) ) {
			return;
		}
		...
		foreach ( $_POST['lvgshop_variation_swatches'] as $attribute_slug => $settings ) {
			$data[ $attribute_slug ] = [
	...
	public function ajax_add_new_attribute_term() {
		if ( ! isset( $_POST['_wpnonce'] ) || ! wp_verify_nonce( $_POST['_wpnonce'], 'lvgshop_variation_swatches_add_term' ) ) {
			wp_send_json_error( esc_html__( 'Wrong request', 'lvgshop-variation-swatches' ) );
		}

		if ( empty( $_POST['attribute_name'] ) || empty( $_POST['attribute_taxonomy'] ) ) {
			wp_send_json_error( esc_html__( 'Not enough data', 'lvgshop-variation-swatches' ) );
		}
		...
		if ( ! taxonomy_exists( $taxonomy ) ) {
			wp_send_json_error( esc_html__( 'Taxonomy is not exists', 'lvgshop-variation-swatches' ) );
		}

		if ( term_exists( $name, $taxonomy ) ) {
			wp_send_json_error( esc_html__( 'This term is already exists', 'lvgshop-variation-swatches' ) );
		}
		...
		if ( ! is_wp_error( $term ) ) {
			wp_send_json_success( [
				'message' => esc_html__( 'Added successfully', 'lvgshop-variation-swatches' ),
	...
	public function dialog_new_term() {
		...
		<div id="lvgshop-variation-swatches-new-term-dialog" class="lvgshop-variation-swatches-dialog hidden" style="display: none">
			<div class="lvgshop-variation-swatches-modal wp-core-ui" tabindex="0" role="dialog">
				<button type="button" class="media-modal-close">
					<span class="media-modal-icon">
						<span class="screen-reader-text"><?php esc_html_e( 'Close dialog', 'lvgshop-variation-swatches' ) ?></span>
					</span>
				</button>
				<div class="lvgshop-variation-swatches-modal__header"><h2><?php esc_html_e( 'Add New Term', 'lvgshop-variation-swatches' ) ?></h2></div>
				<div class="lvgshop-variation-swatches-modal__content">
					<p class="form-field">
						<label>
							<?php esc_html_e( 'Name', 'lvgshop-variation-swatches' ) ?><br>
					...
					<p class="form-field form-field__swatches">
						<span class="lvgshop-variation-swatches__field-color">
							<span class="label"><?php esc_html_e( 'Color', 'lvgshop-variation-swatches' ) ?></span><br>
							<input type="text" name="color" value="">
						</span>
						<span class="lvgshop-variation-swatches__field-image">
							<span class="label"><?php esc_html_e( 'Image', 'lvgshop-variation-swatches' ) ?></span><br>
							<img src="<?php echo esc_url( $default_image ) ?>" data-placeholder="<?php echo esc_url( $default_image ) ?>" width="60" height="60">
							<a href="#" class="button-add-image"><?php esc_html_e( 'Upload', 'lvgshop-variation-swatches' ) ?></a>
							<a href="#" class="button-link button-remove-image delete hidden"><?php esc_html_e( 'Delete', 'lvgshop-variation-swatches' ) ?></a>
							<input type="hidden" name="image" value="">
						</span>
						<span class="lvgshop-variation-swatches__field-label">
							<span class="label"><?php esc_html_e( 'Label', 'lvgshop-variation-swatches' ) ?></span><br>
					...
					<?php wp_nonce_field( 'lvgshop_variation_swatches_add_term', '_wpnonce', false ) ?>
				</div>
				<div class="lvgshop-variation-swatches-modal__footer">
					<button type="button" class="button-add button button-primary"><?php esc_html_e( 'Add New', 'lvgshop-variation-swatches' ) ?></button>
					<span class="lvgshop-variation-swatches-modal__spinner spinner"></span>
					<span class="lvgshop-variation-swatches-modal__message hidden"></span>
				</div>
			</div>

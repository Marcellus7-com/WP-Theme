<?php
namespace Lvgshop\VariationSwatches\Customize;
...
class Customizer {
	 ...
	 * The setting API
	 *
	 * @var \Lvgshop\VariationSwatches\Admin\Settings
	 */
	public $setting_api;
	...
	public function __construct() {
		$this->settings_api = \Lvgshop\VariationSwatches\Admin\Settings::instance();
	...
	public function add_sections( $wp_customize ) {
		$section_name = 'lvgshop_variation_swatches';

		$wp_customize->add_section(
			$section_name,
			[
				'title'    => esc_html__( 'Variation Swatches', 'lvgshop-variation-swatches' ),
				'description' => esc_html__( 'Some of these options can be overidden in product data settings.', 'lvgshop-variation-swatches' ),
				'priority' => 30,
				'panel'    => 'woocommerce',
			]
		);
		...
		$wp_customize->add_control(
			new Size_Control(
				...
				$this->settings_api->get_option_name( 'size' ),
				[
					'label'       => esc_html__( 'Swatches Size', 'lvgshop-variation-swatches' ),
		...
		$wp_customize->add_control(
			$this->settings_api->get_option_name( 'tooltip' ),
			[
				'label'    => esc_html__( 'Enable tooltip', 'lvgshop-variation-swatches' ),
				'section'  => $section_name,
				'type'     => 'checkbox',
			]
		);
		...
		$wp_customize->add_control(
			$this->settings_api->get_option_name( 'auto_button' ),
			[
				'label'    => esc_html__( 'Convert default dropdowns to buttons', 'lvgshop-variation-swatches' ),
				'section'  => $section_name,
				'type'     => 'checkbox',
			]
		);
		...
		$wp_customize->add_control(
			$this->settings_api->get_option_name( 'show_selected_label' ),
			[
				'label'    => esc_html__( "Show the selected attribute's label", 'lvgshop-variation-swatches' ),
				'section'  => $section_name,
				'type'     => 'checkbox',
			]
		);
	...
	public function add_styles() {
		?>
		<style type="text/css">
			.customize-control-lvgshop-variation-swatches-size .customize-control-inside input[type="text"] {
				width: auto;
	...
	public function enqueue_scripts() {
		wp_enqueue_script( 'lvgshop-variation-swatches-customizer', plugins_url( 'assets/js/customizer.js', LVGSHOP_VARIATION_SWATCHES_FILE ), [ 'jquery', 'customize-base' ], '20210722', true );
	}
}

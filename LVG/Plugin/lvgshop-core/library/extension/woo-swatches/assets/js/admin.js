...
jQuery( function( $ ) {
	...
	var LvgshopVariationSwatchesMeta = {
		init: function() {
			$( '.lvgshop-variation-swatches__field-color input' ).wpColorPicker();

			$( document.body )
				.on( 'click', '.lvgshop-variation-swatches__field-image .button-add-image, .lvgshop-variation-swatches__field-image img', this.uploadImage )
				.on( 'click', '.lvgshop-variation-swatches__field-image .button-remove-image', this.removeImage );

			$( '#lvgshop_variation_swatches_data' )
				.on( 'change', '.lvgshop-variaton-swatches__type-field select', this.toggleSwatchesMetabox )
				.on( 'change', '.lvgshop-variaton-swatches__size-field select', this.toggleCustomSizeFields );
			...
			$( '#lvgshop-variation-swatches-new-term-dialog' )
		...
		uploadImage: function( event ) {
			...
			mediaFrame.on( 'select', function () {
				...
				$button.closest( '.lvgshop-variation-swatches__field-image' ).find( 'input[type="hidden"]' ).val( attachment.id );
				$button.siblings( '.button-remove-image' ).removeClass( 'hidden' );

				var attachment_image = attachment.sizes && attachment.sizes.thumbnail ? attachment.sizes.thumbnail.url : attachment.url;
				$button.closest( '.lvgshop-variation-swatches__field-image' ).find( 'img' ).attr( 'src', attachment_image );
			} );
		...
		removeImage: function( event ) {
			...
			$button.closest( '.lvgshop-variation-swatches__field-image' ).find( 'input[type="hidden"]' ).val( '' );
			$button.closest( '.lvgshop-variation-swatches__field-image' ).find( 'img' ).attr( 'src', function() {
		...
		reloadSwatchesPanel: function() {
			...
			$( '#lvgshop_variation_swatches_data' ).load( this_page + ' #lvgshop_variation_swatches_data_inner', function() {
				setTimeout( function() {
					$( '.lvgshop-variation-swatches__field-color input' ).wpColorPicker();
				} );
			} );
		...
		closeDialog: function( event ) {
			...
			$( '#lvgshop-variation-swatches-new-term-dialog' ).hide();
		},
		...
		openNewAttributeDialog: function( event ) {
			...
			var $button = $( this ),
				$dialog = $( '#lvgshop-variation-swatches-new-term-dialog' ),
				data = {
			...
			$( '.form-field__swatches', $dialog ).children().hide().filter( '.lvgshop-variation-swatches__field-' + data.type ).show();
		...
		ajaxAddTerm: function( event ) {
			...
			var $button = $( this ),
				...
				$message = $( '.lvgshop-variation-swatches-modal__message', $dialog ),
			...
			wp.ajax.send( 'lvgshop_variation_swatches_add_term', {
				data   : data,
				error  : function ( res ) {
	...
	LvgshopVariationSwatchesMeta.init();
} );
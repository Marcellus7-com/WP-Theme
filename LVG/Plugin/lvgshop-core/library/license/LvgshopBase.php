<?php
if(!class_exists("Lvgshop_Base")) {
	class Lvgshop_Base {
    	public $key = "56512CF39D325A15";
    	private $product_id = "10";
    	private $product_base = "lvgshop";
    	private $server_host = "https://license.ia.marcellus7.com/wp-json/lvgshop_license/";
		...
		function __construct($plugin_base_file='')
		{
			...
			if($this->has_check_update) {
				if(function_exists("add_action")){
					add_action( 'admin_post_lvgshop_fupc', function(){
					...
					if($this->is_theme){
						...
						add_filter( 'plugin_row_meta', function($links, $plugin_file ){
							if (  plugin_basename( $this->plugin_file ) == $plugin_file ) {
								$links[] = " <a class='edit coption' href='" . esc_url( admin_url( 'admin-post.php' ) . '?action=lvgshop_fupc' ) . "'>Update Check</a>";
							}
							return $links;
						}, 10, 2 );
		 ...
		 public function theme_force_update(){
            ...
            echo wp_kses_post('<h1>'.__("Update Checking..","lvgshop").'</h1>');

 ...
 *
 */
if ( ! class_exists( 'CSF_Field_lvgshop_link' ) ) {
  class CSF_Field_lvgshop_link extends CSF_Fields {
    ...
    echo "<div class='lvgshop_admin_flex'>";
      foreach ( $color_props as $color_prop_key => $color_prop_value ) {
        ...
        if ( ! empty( $args[$color_prop_key] ) ) {
          ...
          echo '<input type="text" name="'. esc_attr( $this->field_name( '['. $color_prop_key .']' ) ) .'" value="'. esc_attr( $value[$color_prop_key] ) .'" class="csf-color lvgshopsingle-'.$this->field['id'].$color_prop_value.'"'. $default_attr . $this->field_attributes() .'/>';
          echo '</div>';
...
jQuery(document).ready(function($){
    $('.csf-color.lvgshopsingle-<?php echo $this->field['id'];?><?php echo $color_prop_value;?>').coloringPick({
	'show_input': false,
	'theme': 'dark',

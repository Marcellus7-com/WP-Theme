 ...
 * @since    1.0
*/
if ( ! function_exists( 'lvgshop_breadcrumbs' ) ) {
    function lvgshop_breadcrumbs() {
        ...
        $current    = apply_filters( 'dm_breadcrumb_current', '<span class="">%s</span>' );
        if ( ( $on_front == 'page' && is_front_page() ) || ( $on_front == 'posts' && is_home() ) ) {
            return;
        }
        $out = '<div class="breadcrumb breadcrumb-text fw-light mb-0">';
        if ( $on_front == "page" && is_home() ) {
            $blog_title = isset( $blog_page ) ? get_the_title( $blog_page ) : __( 'Our Blog', 'lvgshop' );
            $out .= sprintf( $link, home_url(), __( 'Home', 'lvgshop' ) ) . $separator . sprintf( $current, $blog_title );
        } else {
            $out .= sprintf( $link, home_url(), __( 'Home', 'lvgshop' ) );
        }
        if ( is_singular() ) {
            ...
            if ( is_post_type_archive() ) {
                ...
                $out .= $separator . __( '<span>Category</span> <span class="bredcrumb-separator">/</span> ', 'lvgshop' ) . sprintf( $current, $wp_query->queried_object->name );
            } else if ( is_tag() ) {
               $out .= $separator . __( '<span>Tag</span> <span class="bredcrumb-separator">/</span> ', 'lvgshop' ) . sprintf( $current, $wp_query->queried_object->name );
               
            } else if ( is_author() ) {
               
                $out .= $separator . __( '<span>Author</span>', 'lvgshop' );
            } else if ( is_date() ) {
                $out .= $separator;
                if ( is_day() ) {
                    global $wp_locale;
                    $out .=  __( '<span>Day</span>','lvgshop');
                ...
                $out .= $separator . sprintf( $current, __( 'Error 404', 'lvgshop' ) );
            } elseif ( is_search() ) {
			if (have_posts()) {
        ...
        $out .= '</div>';
        echo apply_filters( 'lvgshop_breadcrumbs_out', $out );
    }
}
// Hooks into the Digital Store theme
add_action( 'lvgshop_before_template_header', 'lvgshop_breadcrumbs' );
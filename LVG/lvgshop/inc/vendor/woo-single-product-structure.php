...
if (!function_exists('lvgshop_wc_get_gallery_image_html')) {
    // Copied and modified from woocommerce plugin and wc_get_gallery_image_html helper function.
    function lvgshop_wc_get_gallery_image_html($attachment_id, $main_image = false, $size = 'woocommerce_single') {
...
// pivoo Single Product Hooks
function lvgshop_woo_sale_hook_three() {
    do_action('pivoo_woo_sale_hook');
}
...
function lvgshop_woo_sale_discount_badge() {
    ...
    if ($product->is_type( 'variable' ) && !empty($regular_price) && !empty($sale_price)) { //then this is a variable product
...
add_action('lvgshop_woo_sale_hook_three', 'lvgshop_woo_sale_discount_badge', 7);



function lvgshop_woo_inventory_functions() {
    global $post, $product;
    if ($product->get_stock_quantity()) { // if manage stock is enabled
        if (number_format($product->get_stock_quantity(), 0, '', '') < 10) { // if stock is low
            echo '<span class="lvgshop-remaining-stock red-notice">Only ' . number_format($product->get_stock_quantity(), 0, '', '') . ' in stock</span>';
        } else {
            echo '<span class="lvgshop-remaining-stock green-notice">' . number_format($product->get_stock_quantity(), 0, '', '') . ' in stock</span>';
        }
    }
}

add_action('lvgshop_woo_inventory_hook', 'lvgshop_woo_inventory_functions');
...
if (class_exists('YITH_WCWL')) {
     ...
     */
    function lvgshop_product_wishlist_button() {
        ?>
        <div class="pd-wishlist-btn text-uppercase mt-30">
            <?php echo do_shortcode('[yith_wcwl_add_to_wishlist]'); ?>

        </div>
        <?php
    }
//add_action( 'woocommerce_after_add_to_cart_button', 'lvgshop_product_wishlist_button',10, 0 );
